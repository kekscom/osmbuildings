...
L.BuildingsLayer = (function (global) {
    ...
    LAYER.initialize = function (url) {
        var map = null;
        var canvas = null;

        var _lastX = 0;
        var _lastY = 0;
        var _blockMoveEvent = false; // needed as Leaflet fires moveend and zoomend together

        var _onMove = function () {
            var mp = L.DomUtil.getPosition(this.map._mapPane);
            setCam(
    //                halfWidth - (mp.x - this._lastX),
    //                height    - (mp.y - this._lastY)
            );
            render();
        }
    ...
    LAYER.onAdd = function (map) {
        this.map = map;
        this.canvas = osmb.createCanvas(this.map._panes.overlayPane);

        osmb.setMaxZoom(this.map._layersMaxZoom);

        osmb.setSize(this.map._size.x, this.map._size.y);
        var po = this.map.getPixelOrigin(); // changes on zoom only!
        osmb.setOrigin(po.x, po.y);
        osmb.setZoom(this.map._zoom);
...
//           this.map.on('zoomanim', osmb.onZoom);
        }

        this.map.attributionControl.addAttribution(OSMBuildings.ATTRIBUTION);
        osmb.loadData(); // initially - yes. otherwise?
        osmb.render(); // in case of re-adding this layer
    }
